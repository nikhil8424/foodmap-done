    ...
    <div class="pl-72">
      ...
      <header class="fixed top-0 left-72 right-0 h-20 bg-surface/80 backdrop-blur-xl z-40 flex items-center px-container-margin justify-between border-b border-outline-variant/20">
        <div class="flex items-center gap-2">
          <span class="font-title-md font-bold text-on-surface">Kitchen Management Hub</span>
        </div>

        <div class="flex items-center gap-3">
          <!-- MongoDB Atlas Live Connection Badge -->
          <div class="flex items-center gap-1.5 px-3 py-1 bg-green-50 text-green-700 rounded-full border border-green-200 text-xs font-semibold">
            ...
            <span>Atlas Live Sync</span>
          </div>

          <button
            @click="navigateTo('new_order')"
            class="relative p-2.5 hover:bg-surface-container rounded-full text-on-surface-variant cursor-pointer"
            title="Incoming orders"
          >
            <span class="material-symbols-outlined text-[22px]">notifications</span>
            <span v-if="pendingOrdersCount > 0" class="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-primary rounded-full animate-ping"></span>
          </button>

          <button
            @click="navigateTo('post_new_food')"
            class="flex items-center gap-1.5 bg-primary hover:bg-primary/90 text-on-primary px-3.5 py-2 rounded-xl cursor-pointer transition-all shadow-sm text-xs font-bold"
          >
      ...
      <main class="relative pt-20 min-h-screen bg-background">
        <div class="flex flex-col w-full">
          <div class="flex flex-col gap-6 px-container-margin pb-section-gap max-w-5xl mx-auto w-full pt-6">
            ...
            <section class="flex flex-col gap-4 relative z-10 w-full">
              <div class="flex items-start justify-between w-full">
                ...
                <div class="flex gap-2">
                  <div class="bg-surface-container-lowest px-4 py-2 rounded-2xl border border-outline-variant/20 flex flex-col items-center">
                    <span class="text-xs text-on-surface-variant font-bold">Active Portions</span>
                    <span class="text-lg font-black text-primary">{{ totalPortionsActive }}</span>
                  </div>
                  <div class="bg-surface-container-lowest px-4 py-2 rounded-2xl border border-outline-variant/20 flex flex-col items-center">
                    <span class="text-xs text-on-surface-variant font-bold">Pending Orders</span>
                    <span class="text-lg font-black text-amber-600">{{ pendingOrdersCount }}</span>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                @click="navigateTo('new_order')"
                class="w-full bg-primary/10 border border-primary/30 hover:bg-primary/15 transition-colors rounded-2xl p-4 flex items-center justify-between cursor-pointer shadow-sm"
              >
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center animate-bounce">
                    <span class="material-symbols-outlined text-[20px]">receipt_long</span>
                  </div>
                  <div>
                    <span class="font-bold text-on-surface text-sm block">{{ pendingOrdersCount }} Active Order(s) In Progress!</span>
                    <span class="text-xs text-on-surface-variant">Tap to view live customer orders & update kitchen preparation milestones</span>
                  </div>
                </div>
              ...
              <button
                @click="navigateTo('post_new_food')"
                class="group relative w-full overflow-hidden rounded-3xl bg-primary text-on-primary shadow-lg hover:shadow-xl transition-all duration-300 transform active:scale-[0.99] p-6 flex flex-col sm:flex-row items-center justify-center gap-5 text-center sm:text-left min-h-[140px] cursor-pointer"
              >
                <div class="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center shrink-0 backdrop-blur-md shadow-inner">
                  <span class="material-symbols-outlined text-[32px] text-white">add</span>
                </div>
                <div class="flex flex-col">
                  <span class="text-2xl font-extrabold text-white leading-tight">Post fresh food cooking now</span>
            ...
            <section class="flex flex-col gap-4 relative z-10 w-full mt-2">
              <div class="flex items-center justify-between">
                <div>
                  <h2 class="text-lg font-extrabold text-on-surface">Your Live Radar Batches</h2>
                  <span class="text-xs text-on-surface-variant">All updates sync in real-time across residents</span>
                </div>
                <div class="flex items-center gap-1.5 text-on-surface-variant bg-surface-container px-3 py-1 rounded-full text-xs font-bold">
                  <span class="w-2 h-2 rounded-full bg-primary animate-ping"></span>
                  <span>{{ listings.length }} Dishes Managed</span>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                <div
                  ...
                  class="flex flex-col bg-surface-container-lowest rounded-2xl shadow-sm hover:shadow-md transition-all overflow-hidden border border-outline-variant/20"
                >
                  <div class="relative w-full aspect-[16/9] bg-surface-container-high overflow-hidden">
                    ...
                    <div
                      ...
                      class="absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-bold shadow-sm backdrop-blur-sm flex items-center gap-1.5"
                    >
                      <span :class="!item.isAvailable || item.quantity <= 0 ? 'bg-white' : 'bg-green-500'" class="w-2 h-2 rounded-full"></span>
                    ...
                    <button
                      ...
                      class="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/60 hover:bg-red-600 text-white flex items-center justify-center transition-colors cursor-pointer"
                      title="Delete dish"
                    >
