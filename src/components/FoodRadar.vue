    ...
    <div class="pl-72">
      ...
      <header class="fixed top-0 left-72 right-0 h-20 bg-surface/80 backdrop-blur-xl z-40 flex items-center px-container-margin justify-between border-b border-outline-variant/20">
        ...
        <div class="flex-1 max-w-xl mx-auto">
          <div class="flex items-center bg-surface-container-high/60 rounded-full px-gutter py-2 border border-outline-variant/30 hover:border-primary/40 transition-colors">
            <span class="material-symbols-outlined text-primary mr-2 text-[20px]">search</span>
            <input
              v-model="searchQuery"
              placeholder="Search homemade dishes, Rajma, Poha, Samosa..."
              class="bg-transparent border-none outline-none text-sm text-on-surface font-medium w-full placeholder-on-surface-variant/70"
            />
        ...
        <div class="flex items-center gap-3">
          <!-- Active MongoDB Status Indicator -->
          <div class="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-700 rounded-full border border-green-200 text-xs font-semibold">
          ...
          <button
            @click="navigateTo('order_status')"
            class="flex items-center gap-1.5 bg-surface-container-high hover:bg-surface-container-highest text-on-surface px-4 py-2 rounded-xl cursor-pointer transition-all border border-outline-variant/20 font-label-md text-xs font-bold"
          >
      ...
      <main class="relative pt-20 min-h-screen bg-background">
        ...
        <Transition name="slide-fade">
          <div
            ...
            class="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-primary text-on-primary px-5 py-2.5 rounded-full shadow-lg flex items-center gap-2 text-xs font-bold animate-bounce"
          >
        ...
        <div class="flex flex-col w-full">
          ...
          <div class="px-container-margin py-stack-md flex flex-col md:flex-row justify-between items-start md:items-center gap-stack-md z-10 relative bg-background">
            <div>
              <div class="flex items-center gap-2 mb-1">
                <span class="w-2 h-2 rounded-full bg-primary animate-ping"></span>
                <span class="text-[11px] font-label-sm text-primary tracking-widest uppercase font-bold">Real-time Kitchen Radar</span>
                <span class="text-xs text-on-surface-variant font-medium">• Bhandup West, Mumbai</span>
              </div>
              <h1 class="text-2xl lg:text-3xl font-display-lg text-on-surface font-bold">Food cooking around you now</h1>
            </div>
            ...
            <div class="flex flex-wrap gap-1.5">
              <button
                ...
                class="px-3.5 py-1.5 rounded-full text-xs font-label-md transition-all cursor-pointer"
              >
                All ({{ foods.length }})
              </button>
              <button
          ...
          <div class="flex flex-col lg:flex-row flex-1 min-h-[calc(100vh-180px)]">
            <!-- LEFT: Food Feed -->
            <div class="w-full lg:w-[55%] xl:w-[52%] h-full overflow-y-auto px-container-margin pb-section-gap">
              <!-- Distance & Diet Sub-filters -->
              <div class="flex items-center justify-between gap-stack-md mb-stack-md overflow-x-auto pb-stack-sm hide-scrollbar sticky top-0 bg-background/95 backdrop-blur-md z-20 pt-2 border-b border-outline-variant/10">
                <div class="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    class="whitespace-nowrap px-3 py-1 rounded-full border text-xs uppercase tracking-wider transition-all cursor-pointer"
                  >
                    500m
                  </button>
                  <button
              ...
              <!-- Empty State -->
              <div v-else-if="filteredFoods.length === 0" class="py-16 flex flex-col items-center justify-center text-center p-6 bg-surface-container-low rounded-2xl border border-outline-variant/20">
              ...
              <!-- Feed Grid -->
              <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-stack-md">
                <div
                  v-for="item in filteredFoods"
                  :key="item._id || item.id"
                  @click="openFoodDetail(item)"
                  class="group flex flex-col bg-surface-container-lowest rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 cursor-pointer border border-outline-variant/20"
                >
                  <!-- Dish Photo -->
            ...
            <!-- RIGHT: Interactive Live Map Radar -->
            <div class="hidden lg:flex lg:w-[45%] xl:w-[48%] h-full min-h-[620px] relative rounded-tl-3xl overflow-hidden shadow-[-8px_0_32px_rgba(0,0,0,0.05)] bg-surface-container flex-col">
              ...
              <div class="absolute top-4 left-4 right-4 z-20 flex items-center justify-between pointer-events-none">
                ...
                <div class="bg-surface/90 backdrop-blur-md px-3 py-1 rounded-full shadow-md text-xs font-semibold text-on-surface-variant pointer-events-auto">
                  {{ filteredFoods.length }} Active Kitchen Dishes
                </div>
              </div>
              ...
              <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center pointer-events-none">
                <!-- Outer Range Ring (500m radius representation) -->
                <div class="absolute w-[440px] h-[440px] rounded-full border border-primary/25 bg-primary/5"></div>
                <!-- Scanning Radar Sweep -->
                <div
                  class="absolute w-[440px] h-[440px] rounded-full border border-primary/40 origin-center animate-[spin_10s_linear_infinite]"
